...
SELECT 
    title, rental_duration AS 'Most Rented'
FROM
    film
...
SELECT 
    s.store_id, SUM(p.amount) AS 'Store Earnings'
FROM
    store AS s
        INNER JOIN
...
SELECT 
    ca.name AS 'Top Five Genres', SUM(p.amount) AS 'Revenue'
FROM
    category AS ca
        INNER JOIN
    film_category AS fc ON ca.category_id = fc.category_id
        INNER JOIN
